 ...
 * 
 * <p>
 * On filter initialisation, the class will use Spring's
 * {@link WebApplicationContextUtils#getWebApplicationContext(ServletContext sc)}
 * method to obtain an <code>ApplicationContext</code> instance. It will expect
 * to find the target <code>Filter</code> in this
 ...
 * 
 * <p>
 * An additional initialization parameter, <code>init</code>, is also supported.
 * If set to "<code>lazy</code>" the initialization will take place on the first
 * HTTP request, rather than at filter creation time. This makes it possible to
 * use <code>FilterToBeanProxy</code> with the Spring
 ...
 * 
 * <p>
 * A final optional initialization parameter, <code>lifecycle</code>, determines
 * whether the servlet container or the IoC container manages the lifecycle of
 * the proxied filter. When possible you should write your filters to be managed
 * via the IoC container interfaces such as
 * {@link org.springframework.beans.factory.InitializingBean} and
 * {@link org.springframework.beans.factory.DisposableBean}. If you cannot
 * control the filters you wish to proxy (eg you do not have their source code)
 * you might need to allow the servlet container to manage lifecycle via the
 * {@link javax.servlet.Filter#init(javax.servlet.FilterConfig)} and
 * {@link javax.servlet.Filter#destroy()} methods. If this case, set the
 * <code>lifecycle</code> initialization parameter to
 * <code>servlet-container-managed</code>. If the parameter is any other value,
 * servlet container lifecycle methods will not be delegated through to the
 * proxy.
 * </p>
 * 
 */
